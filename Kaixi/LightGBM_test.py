...
def load_realtime_data():
    ...
    r_forecast = requests.get(
        forecast_url, params={"limit": 20, "order_by": "datetime DESC"}, timeout=10
    )
    df_forecast = pd.DataFrame(r_forecast.json()["results"])[
        ["datetime", "imbalanceprice"]
    ]
    ...
    r_actual = requests.get(
        actual_url, params={"limit": 20, "order_by": "datetime DESC"}, timeout=10
    )
...
def load_training_data():
    ...
    df_forecast["datetime_utc"] = pd.to_datetime(
        df_forecast["datetime_utc"], errors="coerce", utc=True
    )
    ...
    if "datetime_utc" not in df_actual.columns:
        raise KeyError("缺少 datetime_utc 列")
    df_actual["datetime_utc"] = pd.to_datetime(
        df_actual["datetime_utc"], errors="coerce", utc=True
    )
    ...
    if "datetime_utc" not in df_dam.columns:
        raise KeyError("缺少 datetime_utc 列")
    df_dam["datetime_utc"] = pd.to_datetime(
        df_dam["datetime_utc"], errors="coerce", utc=True
    )
...
def create_features_and_target(df_forecast, df_actual, df_dam, is_training=True):
    ...
    # 5分钟滚动平均
    df["forecast_rolling_mean_5min"] = (
        df["forecast_price"].rolling(window=5).mean().shift(1)
    )
    # 15分钟滚动标准差（波动性）
    df["forecast_rolling_std_15min"] = (
        df["forecast_price"].rolling(window=15).std().shift(1)
    )
...
def send_prediction_email(
    current_price, predicted_price, recipient_email="kaixi.yao@outlook.com"
):
    """发送预测结果邮件"""
    # 邮件配置
    ...
    try:
        ...
        msg = MIMEMultipart()
        msg["Subject"] = (
            f'NOX Energy - 价格预测更新 ({next_quarter.strftime("%H:%M")} UTC)'
        )
...
"""
        ...
        part.add_header(
            "Content-Disposition", "attachment", filename="price_prediction.csv"
        )
        msg.attach(part)
...
def predict_with_model(df_forecast, df_actual, df_dam, model, features):
    ...
    target_time = prediction_time + pd.Timedelta(
        minutes=(15 - prediction_time.minute % 15)
    )
...
def evaluate_and_predict(model, X_test, y_test, feature_names):
    ...
    print("\n显示特征重要性...")
    lgb.plot_importance(
        model, max_num_features=20, figsize=(10, 8), importance_type="gain"
    )
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("\n=== 预测结果 ===")
        print(
            f"预测时间: {result['prediction_time'].strftime('%Y-%m-%d %H:%M:%S')} UTC"
        )
